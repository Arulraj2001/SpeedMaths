...
export default function SquaresPage() {
  ...
  const [bookmarks, setBookmarks] = useState<number[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("speedmaths-squares-bookmarks");
      if (saved) {
        try {
  ...
  const toggleBookmark = (n: number) => {
    ...
    if (bookmarks.includes(n)) {
      ...
      toast(`Bookmarked ${n}² = ${n * n}!`, "success");
    }
    setBookmarks(updated);
    localStorage.setItem("speedmaths-squares-bookmarks", JSON.stringify(updated));
  };
  ...
  const handleVoice = (n: number) => {
    ...
    const utterance = new SpeechSynthesisUtterance(`${n} squared is ${n * n}`);
  ...
  // Compile calculations list (1 to 100)
  ...
  const filteredItems = React.useMemo(() => {
    return items.filter((item) => {
      const matchesSearch = 
        ...
        String(item.square).includes(search);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight">Squares (1-100)</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Memorize power squares up to 100. Filter using parameters below.
          </p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-card p-4 rounded-xl border border-border/40 backdrop-blur-sm print:hidden justify-between items-center">
        <div className="relative w-full md:w-80">
          ...
          <input
            type="text"
            placeholder="Search by number or square value..."
            value={search}
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-12 border border-dashed border-border rounded-2xl bg-card/40 print:hidden">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No square computations match your search.</p>
        </div>
      ) : (
